        ...
        <div className="z-20 flex w-full flex-col lg:w-[60%]">
          ...
          <ul className="mt-10 grid gap-10 md:grid-cols-2 lg:mt-20 lg:gap-20">
            {FEATURES.map((feature) => (
              <FeatureItem title={feature.title} key={feature.title} icon={feature.icon} description={feature.description} varient={feature.variant} />
            ))}
          </ul>
        </div>
      </div>
...
const FeatureItem = ({title , description , icon , varient } : FeatureItem) => {
  return (
